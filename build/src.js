...
(function () {
    ...
    var aceThemes = {
        themes: {},
        addTheme: function addTheme(themeName, css) {
            this.themes[themeName] = css.replace(new RegExp("\\.ace-" + themeName, "ig"), ".ace-tm");
        },
        getThemeCSS: function getThemeCSS(themeName) {
    ...
    var chromeid = "gniggljddhajnfbkjndcgnomkddfcial";
    ...
    var queueRoot = "https://reportqueue.herokuapp.com/";
    ...
    function querySelectorAllPromise(elementString) {
        var interval = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 250;
        var maxTrials = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 0;

        return new Promise(function (resolve, reject) {
            var i = 0;
            (function find() {
                var elements = document.querySelectorAll(elementString);
                if (maxTrials > 0 && i > maxTrials) {
    ...
    function querySelectorPromise(elementString) {
        var interval = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 250;
        var maxTrials = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 0;

        return new Promise(function (resolve, reject) {
            querySelectorAllPromise(elementString, interval, maxTrials).then(function (e) {
                return resolve(e[0]);
            }).catch(reject);
        });
    }

    function objectNotEmptyTimer(obj) {
        var interval = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 100;

        return new Promise(function (resolve, reject) {
            (function check() {
                if ((typeof obj === "undefined" ? "undefined" : _typeof(obj)) != "object") {
                    reject(new TypeError(obj + " is not an object"));
    ...
    function HTMLtoKAMarkdown(html) {
        return html.replace(/<pre>\s*<code>([.\s\S]*?)<\/code>\s*<\/pre>/ig, function (match, one) {
            return "```\n" + one + "\n```";
        }).replace(/<code>(.*?)<\/code>/ig, function (match, one) {
            return "`" + one + "`";
        }).replace(/<b>(.*?)<\/b>/ig, function (match, one) {
            return "*" + one + "*";
        }).replace(/<em>(.*?)<\/em>/ig, function (match, one) {
            return "_" + one + "_";
        }).replace(/<a.*?>(.*?)<\/a>/ig, function (match, one) {
            return one;
        }).replace(/<br(?:\s*\/\s*)?>/ig, function () {
            return "\n";
        });
    }
    function KAMarkdowntoHTML(markdown) {
        return markdown.replace(/\`\`\`\s*([.\s\S]*?)\s*\`\`\`/ig, function (match, one) {
            return "<pre><code>" + one + "</code></pre>";
        }).replace(/\`(.+?)\`/ig, function (match, one) {
            return "<code>" + one + "</code>";
        }).replace(/\*(.+?)\*/ig, function (match, one) {
            return "<b>" + one + "</b>";
        }).replace(/_(.+?)_/ig, function (match, one) {
            return "<em>" + one + "</em>";
        }).replace(/\n/ig, function () {
            return "<br>";
        });
    ...
    var CommentLinker = function () {
        function CommentLinker(uok) {
            _classCallCheck(this, CommentLinker);
        ...
        _createClass(CommentLinker, [{
            key: "get",
            value: function get(kaencrypted) {
                return this.commentLinks[kaencrypted];
            }
        }, {
            key: "getTopicId",
            value: function getTopicId(kaencrypted) {
                return this.topicIds[kaencrypted];
            }
        }, {
            key: "getTopicType",
            value: function getTopicType(kaencrypted) {
                return this.topicTypes[kaencrypted];
            }
        }, {
            key: "genURL",
            value: function genURL(page, sort) {
                ...
                getJSON(that.genURL(p, 1), function (data) {
                    ...
                    getJSON(that.genURL(p, 2), function (data) {
                        for (var _i = 0; _i < data.length; _i++) {
                            var _comment = data[_i];
                            that.topicIds[_comment.key] = _comment.focus.id;
                            that.topicTypes[_comment.key] = _comment.focus.kind;
                            that.commentLinks[_comment.key] = _comment.focusUrl + "?qa_expand_key=" + _comment.expandKey;
                        }
                        if (typeof success == "function") {
                            success(that.commentLinks);
                        }
                    });
                });
            }
    ...
    function addGuidelines() {
        querySelectorPromise(".video-discussion").then(function (v) {
            var g = document.getElementsByClassName("main-discussion-guidelines discussion-guidelines")[0],
    ...
    function darkToggleButton() {
        var toolbar = document.getElementsByClassName("toolbar_y6hf3w")[0];
        if (!toolbar) {
            return;
        }
        var rightArea = toolbar.getElementsByClassName("right_piqaq3")[0];
        if (!rightArea) {
            return;
        }
        var outerButtonSpan = document.createElement("span");
        outerButtonSpan.className = "pull-right";
        var innerButtonLink = document.createElement("a");
        ...
        innerButtonLink.addEventListener("mouseover", function () {
            this.style.background = "#484848";
        });
        innerButtonLink.addEventListener("mouseout", function () {
            this.style.background = "#656565";
        });
    ...
    function reportButton() {
        var programDataNotEmpty = objectNotEmptyTimer(programData);
        programDataNotEmpty.then(function (data) {
            var scratchpad = data.scratchpad;
            if (scratchpad && scratchpad.kaid !== kaid && (!KA._userProfileData || !KA._userProfileData.isModerator)) {
                querySelectorPromise(".buttons_vponqv").then(function (buttons) {
    ...
    function showProgramsFlags() {
        ...
        var handleResponse = function handleResponse(a) {
            ...
            if (counter === programLinks.length - 1) {
                for (result = objs.sort(function (a, b) {
                    ...
                    programLinks[a].nextSibling.nextSibling.innerHTML += " \xB7 <span title=\"" + result[a].flags.join("\n") + "\">" + result[a].flags.length + " Flag" + (1 === result[a].flags.length ? "" : "s");
                }
            }
        };
    ...
    function showProgramDates() {
        ...
        if (!programData.scratchpad || !date[0] || !date[0].nextElementSibling) {
            return;
        }
    ...
    function widenProgram() {
        querySelectorPromise(".wrap_xyqcvi").then(function (s) {
            return s.style.setProperty("max-width", "none", "important");
        }).catch(console.error);
    }
    function commentsButtonEventListener() {
        querySelectorPromise(".simple-button.discussion-list-more", 100).then(function (button) {
            return button.addEventListener("click", function () {
                return commentLinkGenerator != null && commentLinkGenerator.next();
            });
        }).catch(console.error);
    }
    ...
    function commentLinks() {
        if (!commentLinkGenerator) {
            return;
        }
        var unalteredComments = document.querySelectorAll(".discussion-item.reply:not(." + extensionCommentClassName + ")");
        for (var i = 0; i < unalteredComments.length; i++) {
            var comment = unalteredComments[i];
            var _url = commentLinkGenerator.get(comment.id);
            if (_url) {
                var metaControls = comment.getElementsByClassName("discussion-meta-controls")[0];
                if (!metaControls) {
                    continue;
                }
    ...
    **/
    function addCommentEditLink(element) {
        ...
        var metaControls = element.getElementsByClassName("discussion-meta-controls")[0],
            modTools = element.getElementsByClassName("mod-tools")[0];

        if (!element || !element.className.includes("reply") || !metaControls || !modTools) {
            return;
        }

        var commentEditLink = document.createElement("a");
        ...
        var discussionControl = document.createElement("div");
        ...
        var orDivide = document.createElement("span");
        ...
        var floatrights = [floatRightSpan.cloneNode(), floatRightSpan.cloneNode(), floatRightSpan.cloneNode()];
        var correspondingElements = [cancel, orDivide, editBtn];
        for (var i = 0; i < floatrights.length; i++) {
        ...
        cancel.addEventListener("click", function (e) {
            var link = e.target;
            var kaencrypted = link.className.substr(commentCancelEditPrefix.length);
            var parentComment = document.getElementById(kaencrypted);
            var discMeta = parentComment.getElementsByClassName("discussion-meta")[0];
            var contentDiv = parentComment.getElementsByClassName("discussion-content")[0];
            var textarea = parentComment.getElementsByTagName("textarea")[0];
            var discussionControl = parentComment.getElementsByClassName("discussion-controls")[0];
        ...
        editBtn.addEventListener("click", function (e) {
            var link = e.target;
            var kaencrypted = /edit-comment-(kaencrypted_.*?)-button/ig.exec(link.className)[1];
            var parentComment = document.getElementById(kaencrypted);
            var discMeta = parentComment.getElementsByClassName("discussion-meta")[0];
            var contentDiv = parentComment.getElementsByClassName("discussion-content")[0];
            var textarea = parentComment.getElementsByTagName("textarea")[0];
            ...
            var focusId = commentLinkGenerator ? commentLinkGenerator.getTopicId(kaencrypted) : KAdefine.require("./javascript/discussion-package/discussion.js").data.focusId,
        ...
        commentEditLink.addEventListener("click", function (e) {
            var link = e.target;
            var kaencrypted = link.className.substr(commentEditClassPrefix.length);
            var parentComment = document.getElementById(kaencrypted);
            var discMeta = parentComment.getElementsByClassName("discussion-meta")[0];
            var contentDiv = parentComment.getElementsByClassName("discussion-content")[0];
            var content = HTMLtoKAMarkdown(contentDiv.innerHTML).trim();
            var textarea = parentComment.getElementsByTagName("textarea")[0];
            var discussionControl = parentComment.getElementsByClassName("discussion-controls")[0];
    ...
    function addCommentEditUI() {
        ...
        if (!KADiscussionPackage || !KADiscussionPackage.data) {
            return;
        }
        if ((!KADiscussionPackage.data.focusId || !KADiscussionPackage.data.focusKind) && !commentLinkGenerator) {
            return;
        }

        var uneditedComments = document.querySelectorAll(".reply:not(." + extensionCommentEditClassName + ")");
        for (var i = 0; i < uneditedComments.length; i++) {
    ...
    function evalFeatures() {
        var projectInfoNotEmpty = objectNotEmptyTimer(programData);
        projectInfoNotEmpty.then(function (projectData) {
            querySelectorPromise(".eval-left > ul").then(function (list) {
                querySelectorPromise('.edit-content-form__left').then(function (tips) {
                    console.log(tips);
    ...
    function locationElm() {
        ...
        if (!locationDiv || !userInfo.userLocation) {
            return;
        }
    ...
    function updateNotifs() {
        querySelectorPromise(".notificationsBadge_16g2pyz", 50).then(function (greenCircle) {
            greenCircle.textContent = KA._userProfileData && KA._userProfileData.countBrandNewNotifications !== 0 ? KA._userProfileData.countBrandNewNotifications : 1;
        }).catch(console.error);
    }

    function stopNotifOverflow() {
        querySelectorPromise(".scrollDropdown_1jabbia").then(function (notifList) {
            return notifList.style.setProperty("overflow-x", "hidden");
        }).catch(console.error);
    }
    ...
    function deleteNotif() {
        ...
        for (var i = 0; i < notifList.length; i++) {
            if (notifList[i].className.length > 0) {
                continue;
            }
            ...
            if (linkElm.className === "loadingSpinner_18tyv6y") {
                continue;
            }
            ...
            var deleteSVG = document.createElementNS(svgNamespace, "svg");
            ...
            var delteSVGPath = document.createElementNS(svgNamespace, "path");
    ...
    function getProfileData() {
        ...
        if (profile.length < 1 || !userPrograms.scratchpads) {
            return;
        }
        var table = profile[0];
    ...
    function reportUserButton() {
        ...
        var userDataNotEmpty = objectNotEmptyTimer(userInfo);
        userDataNotEmpty.then(function (userInfo) {
            if (userInfo.kaid == kaid) {
                return;
            }
            querySelectorAllPromise(".profile-widget", 100).then(function (e) {
                var discussionWidget = [].concat(_toConsumableArray(e)).filter(function (e) {
                    return e.querySelector("a.profile-widget-view-all[href$=\"/discussion\"]");
                })[0];
                ...
                if (discussionWidget) {
                    ...
                    var dWidget = document.getElementById("discussion-widget");
                    var widget = discussionWidget.getElementsByClassName("profile-widget-contents")[0];
    ...
    function centerPoints() {
        querySelectorPromise(".energy-points-badge").then(function (pointsBadge) {
