...
(function () {
    ...
    function querySelectorPromise(elementString, interval = 250, maxTrials = 0) {
        return new Promise((resolve, reject) => {
            querySelectorAllPromise(elementString, interval, maxTrials).then(e => resolve(e[0])).catch(reject);
        });
    }
    ...
    function buildQuery(params) {
        let ret = "",
            v = 0;
        for (let i in params) ret += `${encodeURIComponent(i)}=${encodeURIComponent(params[i] + "") + (v++ != Object.keys(params).length - 1 ? "&" : "")}`;
        return ret;
    }
    ...
    function updateNotifs() {
        querySelectorPromise(".notificationsBadge_16g2pyz", 50).then(greenCircle => {
    ...
    function stopNotifOverflow() {
        querySelectorPromise(".scrollDropdown_1jabbia").then(notifList => notifList.style.setProperty("overflow-x", "hidden")).catch(console.error);
    }

    /***  Programs no longer have a max width. ***/
    function widenProgram() {
        querySelectorPromise(".wrap_xyqcvi").then(s => s.style.setProperty("max-width", "none", "important")).catch(console.error);
    }

    /*** Add back the community guidelines next to dicussion under programs. ***/
    function addGuidelines() {
        querySelectorPromise(".video-discussion").then(v => {
    ...
    function reportUserButton() {
        ...
        userDataNotEmpty.then(userInfo => {
            ...
            querySelectorAllPromise(".profile-widget", 100).then(e => {
    ...
    function commentsButtonEventListener() {
        querySelectorPromise(".simple-button.discussion-list-more", 100).then(button => button.addEventListener("click", () => commentLinkGenerator != null && commentLinkGenerator.next())).catch(console.error);
    }
    ...
    function reportButton() {
        ...
        programDataNotEmpty.then(data => {
            ...
            if (scratchpad && scratchpad.kaid !== kaid && (!KA._userProfileData || !KA._userProfileData.isModerator)) {
                querySelectorPromise(".buttons_vponqv").then(buttons => {
